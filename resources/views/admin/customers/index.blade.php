...
@section('content')
    <div class="flex flex-col min-h-screen lg:h-screen bg-[#0A0A0A]" x-data="{ openDraw: false }">
        ...
        <div class="p-4 sm:p-8 flex-1 overflow-y-auto custom-scroll space-y-6 lg:space-y-8">
            ...
            <div class="flex flex-col gap-6">
                ...
                @if(session('winners'))
                    <div class="bg-[#C9A84C]/20 border border-[#C9A84C] p-6 rounded-[2.5rem] animate-bounce shadow-[0_0_50px_rgba(201,168,76,0.2)]">
                        <h2 class="text-[#C9A84C] font-black italic uppercase text-center text-lg mb-2">🎉 Ganhadores do Sorteio!</h2>
                        <div class="flex flex-wrap justify-center gap-4">
                            @foreach(session('winners') as $winner)
                                <span class="bg-black text-white px-4 py-2 rounded-full font-black uppercase text-[10px] border border-[#C9A84C]">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <button @click="openDraw = true" class="bg-[#C9A84C] text-black px-6 py-3 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-[#C9A84C]/10 flex items-center justify-center gap-2 hover:scale-105 transition-all">
                        ...
                        <div class="flex gap-2 bg-zinc-900/50 p-1 rounded-xl border border-zinc-800 overflow-x-auto no-scrollbar justify-center lg:justify-start">
                            ...
                            <a href="{{ route('admin.customers', ['filter' => 'hoje']) }}" class="whitespace-nowrap px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all {{ $filter == 'hoje' ? 'bg-[#C9A84C] text-black' : 'text-zinc-500 hover:text-white' }}">Hoje</a>
                            <a href="{{ route('admin.customers', ['filter' => 'amanha']) }}" class="whitespace-nowrap px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all {{ $filter == 'amanha' ? 'bg-[#C9A84C] text-black' : 'text-zinc-500 hover:text-white' }}">Amanhã</a>
                            <a href="{{ route('admin.customers', ['filter' => 'semana']) }}" class="whitespace-nowrap px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all {{ $filter == 'semana' ? 'bg-[#C9A84C] text-black' : 'text-zinc-500 hover:text-white' }}">7 Dias</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="max-w-xl mx-auto lg:mx-0 w-full">
                    <form action="{{ route('admin.customers') }}" method="GET" class="relative group">
                        ...
                        <div class="absolute inset-y-0 left-4 flex items-center pointer-events-none transition-all group-focus-within:text-[#C9A84C]">
                            <i class="fas fa-search text-xs"></i>
                        </div>
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="ENCONTRAR CLIENTE POR NOME..."
                               class="w-full bg-[#141414] border border-zinc-800 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl py-4 pl-12 pr-4 focus:border-[#C9A84C] focus:ring-0 outline-none transition-all placeholder-zinc-700">
                    </form>
                </div>
            </div>
                    ...
                    <table class="w-full text-left border-separate border-spacing-y-2 min-w-[700px]">
                        ...
                        @forelse($customers as $cliente)
                            <tr class="bg-zinc-900/20 hover:bg-zinc-900/50 transition-all group">
                                <td class="py-4 px-4 rounded-l-2xl border-y border-l border-zinc-800/50 group-hover:border-[#C9A84C]/30">
                                    <div class="flex items-center gap-3">
                                        <div class="w-9 h-9 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-[10px] font-black text-[#C9A84C] shrink-0">
                                ...
                                <td class="py-4 px-4 border-y border-zinc-800/50 group-hover:border-[#C9A84C]/30 font-mono text-[#C9A84C] whitespace-nowrap">
                                    {{ $cliente->whatsapp }}
                                </td>
                                <td class="py-4 px-4 border-y border-zinc-800/50 group-hover:border-[#C9A84C]/30">
                                    @if($cliente->next_date)
                                        <div class="flex flex-col gap-2 min-w-[120px]">
                                            <div class="text-zinc-400 text-[10px]">
                                                ...
                                                às <span class="text-[#C9A84C] font-bold">{{ $cliente->next_time }}</span>
                                            </div>
                                ...
                                <td class="py-4 px-4 text-right rounded-r-2xl border-y border-r border-zinc-800/50 group-hover:border-[#C9A84C]/30">
                                    @php
                                        ...
                                        $reviewMsg  = $reviewLink
                                            ? urlencode("Olá {$cliente->name}! 😊 Adoraria saber como foi sua experiência no Lima's Studio Barber! Deixa sua avaliação e garante 5% de desconto no próximo corte 👇\n{$reviewLink}")
                                            : null;
                                    @endphp
                                    <div class="flex justify-end gap-2">
                                        ...
                                        @if($phone && $reviewMsg)
                                            <a href="https://wa.me/55{{ $phone }}?text={{ $reviewMsg }}" target="_blank"
                                               class="bg-[#C9A84C]/10 hover:bg-[#C9A84C] text-[#C9A84C] hover:text-black w-8 h-8 rounded-lg flex items-center justify-center transition-all border border-[#C9A84C]/30"
        ...
        <div x-show="openDraw" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-sm" x-cloak>
            <div class="bg-[#141414] border border-zinc-800 p-8 rounded-[2.5rem] w-full max-w-md shadow-2xl relative" @click.away="openDraw = false">
                <h2 class="text-white font-black italic uppercase tracking-tighter text-xl mb-6 flex items-center gap-3">
                    <i class="fas fa-star text-[#C9A84C]"></i> Configurar Sorteio
                </h2>
                ...
                <form action="{{ route('admin.customers.draw') }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <label class="relative cursor-pointer">
                                ...
                                <div class="p-4 bg-zinc-900 border border-zinc-800 rounded-2xl text-center peer-checked:border-[#C9A84C] peer-checked:text-[#C9A84C] transition-all">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="quantity" value="1" min="1" max="10"
                               class="w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-4 text-white text-xs focus:border-[#C9A84C] outline-none font-black">
                    </div>
                    ...
                    <div class="pt-4 flex gap-3">
                        ...
                        <button type="submit" class="flex-[2] bg-[#C9A84C] text-black py-4 rounded-2xl font-black text-[10px] uppercase italic shadow-lg shadow-[#C9A84C]/10">Sortear Agora</button>
                    </div>
                </form>
            </div>
        </div>
