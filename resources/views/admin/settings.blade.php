...
@section('content')
    <div class="p-4 sm:p-8 space-y-8 bg-[#0A0A0A] min-h-screen">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="bg-[#141414] p-8 rounded-[2.5rem] border border-zinc-800 shadow-2xl text-center relative overflow-hidden group">
                    <div class="absolute top-0 left-0 w-full h-1 bg-[#C9A84C]"></div>

                    <div class="relative inline-block mb-6">
                        {{-- Avatar dinâmico puxando o nome do Admin logado --}}
                        <img src="{{ asset('images/Logo-Limas.png') }}" alt="Lima's Studio Barber"
                             class="object-contain p-1 bg-[#0A0A0A] w-24 h-24 rounded-3xl border-2 border-zinc-800 shadow-2xl mx-auto">
                        <button class="absolute -bottom-2 -right-2 bg-zinc-900 text-[#C9A84C] p-2 rounded-xl border border-zinc-800 hover:bg-[#C9A84C] hover:text-black transition-all">
                            <i class="fas fa-camera text-xs"></i>
                        </button>
                    </div>

                    <h3 class="text-white font-black italic uppercase text-lg leading-none">{{ auth()->user()->name }}</h3>
                    <p class="text-[#C9A84C] text-[10px] font-bold uppercase tracking-[0.2em] mt-2">Administrador Master</p>
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                {{-- DADOS DA BARBEARIA --}}
                <div class="bg-[#141414] p-6 sm:p-8 rounded-[2.5rem] border border-zinc-800 shadow-2xl">
                    <h4 class="text-white font-black italic uppercase text-xs tracking-widest mb-8 flex items-center gap-3">
                        <i class="fas fa-store text-[#C9A84C]"></i> Dados da Unidade
                    </h4>
                    ...
                    <form action="{{ route('admin.settings.update') }}" method="POST" class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="unit_name" value="{{ $barbearia['nome'] }}" required
                                   class="w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-4 text-sm text-white focus:border-[#C9A84C] outline-none transition-all">
                        </div>
                        <div class="space-y-2">
                        ...
                        <div class="sm:col-span-2 flex justify-end">
                            <button type="submit" class="w-full sm:w-auto bg-[#C9A84C] text-black px-8 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg active:scale-95 transition-all">Salvar Unidade</button>
                        </div>
                    </form>
                </div>
                ...
                <div class="bg-[#141414] p-6 sm:p-8 rounded-[2.5rem] border border-zinc-800 shadow-2xl">
                    <h4 class="text-white font-black italic uppercase text-xs tracking-widest mb-8 flex items-center gap-3">
                        <i class="fab fa-google text-[#C9A84C]"></i> Cupom de Avaliação Google
                    </h4>
                    ...
                    <form action="{{ route('admin.settings.update') }}" method="POST" class="space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-3 bg-zinc-900 border border-zinc-800 rounded-2xl px-4 py-3">
                                    ...
                                    <input type="checkbox" name="review_coupon_active" value="1" id="review_coupon_active"
                                           ...
                                           class="w-4 h-4 accent-[#C9A84C] cursor-pointer">
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="w-full sm:w-auto bg-[#C9A84C] text-black px-8 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg active:scale-95 transition-all">Salvar Cupom</button>
                        </div>
                    </form>
                </div>
                ...
                <div class="bg-[#141414] p-6 sm:p-8 rounded-[2.5rem] border border-zinc-800 shadow-2xl">
                    <h4 class="text-white font-black italic uppercase text-xs tracking-widest mb-8 flex items-center gap-3">
                        <i class="fas fa-shield-alt text-[#C9A84C]"></i> Segurança e Senha
                    </h4>
                    ...
                    <form action="{{ route('admin.settings.update') }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <input type="password" name="password" placeholder="••••••••" required
                                       class="w-full bg-zinc-900 border rounded-2xl p-4 text-sm text-white focus:border-[#C9A84C] outline-none transition-all @error('password') border-red-500 @else border-zinc-800 @enderror">
                                @error('password') 
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="w-full sm:w-auto bg-[#C9A84C] text-black px-8 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-opacity-90 shadow-lg active:scale-95 transition-all">Atualizar Senha</button>
                        </div>
                    </form>
                </div>
            </div>
