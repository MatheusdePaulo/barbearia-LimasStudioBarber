...
@section('content')
    ...
    <div class="p-4 sm:p-8 space-y-6 lg:space-y-8 bg-[#0A0A0A] min-h-screen" x-data="{ openModal: false, activeProduct: {} }">
                    ...
                    <table class="w-full text-left">
                        ...
                        @foreach($products as $produto)
                            <tr class="border-b border-zinc-800/50 hover:bg-zinc-900/30 transition-all group">
                                ...
                                <td class="py-4 text-[#C9A84C] font-black italic text-sm">R$ {{ number_format($produto->price, 2, ',', '.') }}</td>
                                ...
                                <td class="py-4 text-right">
                                    <button @click="openModal = true; activeProduct = { id: '{{ $produto->id }}', name: '{{ $produto->name }}', price: '{{ $produto->price }}', stock: '{{ $produto->stock }}' }"
                                            class="bg-zinc-800 group-hover:bg-[#C9A84C] group-hover:text-black px-4 py-2.5 rounded-xl text-[9px] font-black uppercase transition-all tracking-widest">
                                        Ajustar Valores
                                    </button>
                                </td>
                            </tr>
                        ...
                        <div class="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-4 flex flex-col gap-4">
                            <div class="flex gap-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-[#C9A84C] font-black italic text-sm">R$ {{ number_format($produto->price, 2, ',', '.') }}</p>
                                    <div class="mt-2">
                            ...
                            <button @click="openModal = true; activeProduct = { id: '{{ $produto->id }}', name: '{{ $produto->name }}', price: '{{ $produto->price }}', stock: '{{ $produto->stock }}' }"
                                    class="w-full bg-zinc-800 text-white py-3.5 rounded-xl text-[9px] font-black uppercase tracking-widest border border-zinc-700 active:bg-[#C9A84C] active:text-black transition-all">
                ...
                <form :action="`/admin/products/${activeProduct.id}`" method="POST" class="space-y-5">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" step="0.01" name="price" x-model="activeProduct.price"
                               class="w-full bg-zinc-900 border-zinc-800 rounded-2xl py-4 px-5 text-white text-sm focus:border-[#C9A84C] focus:ring-0 transition-all font-mono">
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 pt-4">
                        ...
                        <button type="submit"
                                class="order-1 sm:order-2 flex-1 bg-[#C9A84C] text-black py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-[#C9A84C]/10">Confirmar</button>
                    </div>
                </form>
            </div>
        </div>
